...
- `RACKPAD_TAG=1.2.3-beta.4` pins the Docker image to a known release. Git tags use
...
- `RACKPAD_TAG`: release version to run, for example `1.2.3-beta.4`, or `latest` for
...
```bash
sudo git checkout v1.2.3-beta.4
sudo docker compose up --build -d
```
