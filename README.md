...
- Controller API integrations for Proxmox VE, UniFi Network, TP-Link Omada, OPNsense, and Dockhand with encrypted stored credentials, background connection-status refresh, read-only device and container previews, review-first VLAN/subnet apply into IPAM, DHCP range previews, and live Proxmox staging into the import wizard
