...
Docker synchronization, SNMP traps, and session cleanup in-process. The normal
distribution is one hardened container. There is no ORM, queue, external cache,
or separate worker tier.
...
High-blast paths are `server/db.ts`, `server/routes/admin.ts`, `server/app.ts`,
...
3. Browser requests pass host/origin, authentication, and per-handler
   authorization before routes access SQLite.
...
5. Outbound monitor/alert HTTP flows pass through the DNS-pinned network guard.
...
- Public paths are a small allowlist in `server/app.ts`; changing it is restricted.
