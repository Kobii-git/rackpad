...
export function startSnmpTrapReceiver() {
  ...
  socket.on('message', (packet, remote) => {
    void handleTrapPacket(packet, remote.address).catch((error) => {
      status.lastError = error instanceof Error ? error.message : 'Trap processing failed.'
    })
  })
...
function resolveTrapSource(sourceIp: string): TrapSourceMapping {
  const existing = db
    .prepare('SELECT * FROM snmpTrapSources WHERE sourceIp = ? ORDER BY lastTrapAt DESC LIMIT 1')
  ...
  if (existing) {
    return {
      ...
      credentialId: existing.credentialId ? String(existing.credentialId) : null,
    }
  }

  const device = db
    .prepare('SELECT id, labId FROM devices WHERE managementIp = ? ORDER BY hostname LIMIT 1')
...
function communityMatches(actual: string | null | undefined, expected: string | null | undefined) {
...
export function listSnmpTrapLog(options: {
  ...
  return (db.prepare(sql).all(...params) as Record<string, unknown>[]).map(parseTrapLogRow)
}

export function listSnmpTrapSources(labId?: string) {
  const rows = labId
    ? db
        .prepare('SELECT * FROM snmpTrapSources WHERE labId = ? ORDER BY sourceIp')
        .all(labId)
    : db.prepare('SELECT * FROM snmpTrapSources ORDER BY labId, sourceIp').all()
...
export function updateSnmpTrapSource(
  ...
  const existing = db.prepare('SELECT * FROM snmpTrapSources WHERE id = ?').get(id) as
    | Record<string, unknown>
    | undefined
  if (!existing) return null

  const deviceId =
    input.deviceId === undefined ? (existing.deviceId as string | null) : input.deviceId
  if (deviceId) {
    const device = db
  ...
  return parseTrapSourceRow(
    db.prepare('SELECT * FROM snmpTrapSources WHERE id = ?').get(id) as Record<string, unknown>,
  )
}
