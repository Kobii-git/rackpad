...
function passwordToKey(
  ...
  const hash = protocol === 'MD5'
    ? createHash('md5').update(digestInput).digest()
    : createHash('sha1').update(digestInput).digest()

  const localized = protocol === 'MD5'
    ? createHash('md5').update(Buffer.concat([hash, engineId, hash])).digest()
    : createHash('sha1').update(Buffer.concat([hash, engineId, hash])).digest()

  return localized
}

function localizedPrivKey(
  protocol: SnmpV3AuthProtocol,
  password: string,
  engineId: Buffer,
) {
...
function buildScopedPdu(contextEngineId: Buffer, contextName: string, pdu: Buffer) {
  return berSequence(
    Buffer.concat([
...
function buildUsmSecurityParameters(
  engineId: Buffer,
  boots: number,
  time: number,
  user: string,
...
function buildSnmpV3Message(options: {
  msgId: number
  flags: number
...
function insertAuthParameters(message: Buffer, authOffset: number, authParams: Buffer) {
  const next = Buffer.from(message)
  authParams.copy(next, authOffset)
  return next
}

function findAuthParameterOffset(securityParametersValue: Buffer, user: string) {
  let offset = 0
...
function encodeSnmpV3Request(
  ...
  if (usePriv) {
    flags |= 0x03
    const authKey = passwordToKey(session.authProtocol, session.authPassword, engine.id)
    const privKey = localizedPrivKey(session.authProtocol, session.privPassword, engine.id)
    const encrypted = encryptScopedPdu(scopedPdu, privKey, engine.boots, engine.time)
...
function sendSnmpV3(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      finish(() => {
        reject(
          new Error(`SNMPv3 ${session.host}:${session.port} timed out from the Rackpad server.`),
        )
      })
    }, session.timeoutMs)
    ...
    socket.once('message', (packet) => {
      finish(() => {
        try {
          ...
          resolve(parseSnmpV3ResponsePacket(packet, session, discovered ?? engine, requestId))
        } catch (error) {
          reject(error)
        }
      })
...
async function discoverEngine(session: SnmpV3Session) {
  ...
  const packet = await new Promise<Buffer>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      finish(() => reject(new Error(`SNMPv3 engine discovery timed out for ${session.host}.`)))
    }, session.timeoutMs)
    socket.once('error', (error) => finish(() => reject(error)))
  ...
  const discovered = parseEngineFromResponse(packet)
  if (!discovered) {
    throw new Error('SNMPv3 engine discovery did not return an authoritative engine ID.')
  }
  engineCache.set(cacheKey(session), discovered)
  return discovered
}
...
export async function snmpV3Request(
  ...
  const requestId = Math.floor(Math.random() * 0x7fffffff)
  const message = encodeSnmpV3Request(session, engine, normalizedOid, requestId, false, mode)
  try {
...
export function snmpV3Get(session: SnmpV3Session, oid: string): Promise<SnmpValue> {
  return snmpV3Request(session, oid, 'get')
}

export function snmpV3GetNext(session: SnmpV3Session, oid: string): Promise<SnmpValue> {
